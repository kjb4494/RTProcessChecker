...
for (pid, name) in dbg.enumerate_processes():
    ...
    if name.lower() == processName:
        ...
        dbg.attach(pid)
        print
        "Saves a process handle in self.h_process of pid[%d]" % pid
        ...
        if hookAddress:
            ...
            print
            "sets a breakpoint at the designated address : 0x%08x" % hookAddress
            break
        else:
            print
...
if isProcess:
    print
    "wating for occurring debugger event"
    dbg.run()
else:
    print
    "[Error] : There in no process [%s]" % processName
    sys.exit(-1)
